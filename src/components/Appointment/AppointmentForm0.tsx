...
const AppointmentForm = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        // setNotification({ message: "Error loading services", type: "error" });
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasErrors) {
      const formattedData = {
        dentalServiceId: formData.dentalServiceId?.id ?? 0,
      ...
      <FormContainer onSubmit={handleSubmit}>
        ...
        <FieldContainer>
          ...
          <ServiceDropdown
            ...
            onSelect={(dentalServiceId) => setFormData({ ...formData, dentalServiceId })}
          />
        </FieldContainer>
